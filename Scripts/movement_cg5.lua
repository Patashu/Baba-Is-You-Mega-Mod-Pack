function movecommand(ox,oy,dir_,playerid_)
	...
	while (take <= takecount) or finaltake do
		...
		if (finaltake == false) then
			...
			if (take == 2) then
				activemod.addFindMoves(moving_units)
			end

			if (take == 3) then
				activemod.addEvilAndRepentMoves(moving_units)
			end
		...
		while (done == false) do
			...
			for i,data in ipairs(moving_units) do
				...
				if (data.state == state) and (data.moves > 0) and (data.unitid ~= 0) then
					...
					if (x ~= -1) and (y ~= -1) then
						...
						local ox,oy = ndrs[1],ndrs[2]
						local rx,ry,rdir = x+ox,y+oy,dir
						if data.unitid ~= 2 then
							rx,ry,rdir = activemod.getadjacenttile(data.unitid,x,y,dir)
						end
						local pushobslist = {}
						...
						while (result_check == false) and (solved == false) do
							if (result == 0) then
								...
								local swaps = findfeatureat(nil,"is","swap",rx,ry)
								if (swaps ~= nil) then
								...
								for c,pobs in ipairs(pullobs) do
									if (pobs < -1) or (pobs > 1) then
										...
										-- Go backwards from the puller to determine the direction the pullee needs to be pulled.
										-- This is not necessarily the direction the puller is moving.
										local _, _, rdir = activemod.getadjacenttile(data.unitid, x, y, activemod.reversedir(dir))
										rdir = activemod.reversedir(rdir)
										local hm = trypush(paobs,rdir,true,x,y,data.reason,data.unitid)
										if (hm == 0) then
								...
								for c,pobs in ipairs(finalpullobs) do
									...
									local _, _, rdir = activemod.getadjacenttile(data.unitid, x, y, activemod.reversedir(dir))
									rdir = activemod.reversedir(rdir)
									dopush(pobs,rdir,true,x,y,data.reason,data.unitid)
								end
							...
							elseif (result == 1) then
								if (state < 1) then
									...
									for c,pushobs in ipairs(pushobslist) do
										local hm = trypush(pushobs,dir,false,x,y,data.reason)
										if (hm == 0) then
									...
									if (result == 1) then
										for c,pushobs in ipairs(finalpushobs) do
											pushedunits = {}
											dopush(pushobs,rdir,false,x,y,data.reason)
										end
										result = 0
									end
								end
			...
			if (#movelist > 0) then
				for i,data in ipairs(movelist) do
					move(data[1],data[4],data[5])
				end
			end

			-- If one of our still_moving units changed ID as a result of fancy-animation warping, update the ID
			-- in still_moving as well. Otherwise the pre-warped version will be moved multiple times,
			-- causing it to be duplicated.
			for _,data in ipairs(still_moving) do
				for _,idchange in ipairs(activemod.warpedidchanges) do
					if data.unitid == idchange[1] then
						data.unitid = idchange[2]
					end
				end
			end
...
function check(unitid,x,y,dir,pulling_,reason)
	...
	local rx,ry,rdir = activemod.getadjacenttile(unitid,x,y,dir_)
	
	local result = {}
	local results = {}
	local specials = {}
	
	local emptystop = hasfeature("empty","is","stop",2,rx,ry)
	local emptypush = hasfeature("empty","is","push",2,rx,ry)
	local emptypull = hasfeature("empty","is","pull",2,rx,ry)
	local emptyswap = hasfeature("empty","is","swap",2,rx,ry)
	...
	if (#obs > 0) then
		...
		if (eat ~= nil) and (pulling == false) then
			local eats = hasfeature(name,"eat",bname,unitid,rx,ry)
			
			if (eats ~= nil) and (issafe(2,rx,ry) == false) then
				valid = false
		...
		if (lockpartner ~= "") and (pulling == false) then
			local partner = hasfeature(bname,"is",lockpartner,2,rx,ry)
			
			if (partner ~= nil) and ((issafe(2,rx,ry) == false) or (issafe(unitid) == false)) then
				valid = false
				table.insert(specials, {2, "lock"})
			end
		end
		
		local weak = hasfeature(bname,"is","weak",2,rx,ry)
		if (weak ~= nil) and (pulling == false) then
			if (issafe(2,rx,ry) == false) then
				--valid = false
...
function trypush(unitid,dir,pulling_,x_,y_,reason,pusherid,alreadypushing_)
	local x,y = 0,0
	local unit = {}
	local name = ""
	...
	local rx,ry,rdir = activemod.getadjacenttile(unitid,x,y,dir)
			...
			while (done == false) do
				...
				elseif (hm == 1) or (hm == -1) then
					...
					for _,alreadypushedid in ipairs(alreadypushing) do
						if hm == alreadypushedid then
							alreadypushed = true
							break
						end
					end
					if (pulling == false and alreadypushed == false) then
						table.insert(alreadypushing,unitid)
						hm = trypush(hm,rdir,pulling,rx,ry,reason,unitid,alreadypushing)
...
function dopush(unitid,dir,pulling_,x_,y_,reason,pusherid,alreadypushing_)
	local ndrs = ndirs[dir + 1]
	local ox, oy = ndrs[1], ndrs[2]
	...
	if pulling then
		...
		if swap then
			local swapthese = findallhere(rx,ry)
	...
	if (HACK_MOVES < 10000) then
		...
		while (finaldone == false) and (HACK_MOVES < 10000) do
			if (result == 0) then
				table.insert(movelist, {unitid,ox,oy,dir,specials}) -- XYZ
				...
				if (pulling == false) then
					for i,obs in ipairs(pullhmlist) do
						if (obs < -1) or (obs > 1) and (obs ~= pusherid) then
							...
							-- Go backwards from the puller to determine the direction the next object to chain-pull
							-- and the direction to pull it.
							local rx, ry, rdir = activemod.getadjacenttile(data.unitid, x, y, activemod.reversedir(dir))
							rdir = activemod.reversedir(rdir)

							local pid = tostring(rx + ry * roomsizex) .. tostring(obs)
							...
							if (pushedunits[pid] == nil) then
								...
								if not alreadypushed then
									hm = dopush(obs,rdir,true,rx,ry,reason,unitid,alreadypushing)
								end
							end
						end
					end
			...
			elseif (result == 1) then
				for i,v in ipairs(hmlist) do
					if (v ~= -1) and (v ~= 0) and (v ~= 1) then
						local pid = tostring(rx + ry * roomsizex) .. tostring(v)
						
						if (pulling == false) or (pulling and (hms[i] ~= pusherid)) and (pushedunits[pid] == nil) then
							pushedunits[pid] = 1
							local alreadypushed = false
							for _,alreadypushedid in ipairs(alreadypushing) do
								if v == alreadypushedid then
									alreadypushed = true
									break
								end
							end
							if not alreadypushed then
								hm = dopush(v,rdir,false,rx,ry,reason,unitid,alreadypushing)
							end
						end
					end
				end
		...
		if pulling and (HACK_MOVES < 10000) then
			...
			for i,obs in pairs(hmlist) do
				if (obs < -1) or (obs > 1) then
					-- Go backwards from the puller to determine the next object to chain-pull and which direction to		
					-- pull it in.		
					local rx, ry, rdir = activemod.getadjacenttile(unitid, x, y, activemod.reversedir(dir))		
					rdir = activemod.reversedir(rdir)

					if (obs ~= 2) then
						-- This line seems redundant? It seems to create a duplicate of the row created in the line marked "-- XYZ"
						-- Maybe it's here for some special reason.
						table.insert(movelist, {obs,ox,oy,rdir,specials})
						pushsound = true
						--move(obs,ox,oy,dir,specials)
					end
					...
					if (pushedunits[pid] == nil) then
						...
						if not alreadypushed then
							hm = dopush(obs,rdir,pulling,rx,ry,reason,unitid,alreadypushing)
						end
					end
				end
			end
...
function move(unitid,dir,specials_,instant_,simulate_,preservedirection)
	...
	if (unitid ~= 2) then
					...
					if (bx ~= rx) or (by ~= ry) then
						...
						for c,d in ipairs(movelist) do
							if (d[1] == b) then
								...
								--print(tostring(nx) .. "," .. tostring(ny) .. " --> " .. tostring(rx) .. "," .. tostring(ry) .. " (" .. tostring(bx) .. "," .. tostring(by) .. ")")
								if (nx ~= rx) or (ny ~= ry) then
									dodge = true
								end
							end
						end
		...
		if (gone == false) and (simulate == false) then
			local finalfacing = rdir
			if preservedirection then
				finalfacing = unit.values[DIR]
			end

			if instant then
				update(unitid,rx,ry,finalfacing)
				MF_alert("Instant movement on " .. tostring(unitid))
			else
				if #warps > 0 and activemod.fancyWarpAnimation then
					local newunitid = activemod.dofancyanimation(unitid, rx, ry, finalfacing, warps)
					-- Keep track of the fact that the object with that unitid "really" has this new unitid now.
					-- This is necessary when a single unit moves multiple times in the same take (e.g. keke is move and move and move and move and wrap),
					-- because otherwise the original, pre-warped version gets moved multiple times, causing units to be duplicated.
					table.insert(activemod.warpedidchanges, {unitid, newunitid})
				else
					addaction(unitid,{"update",rx,ry,finalfacing})
				end
			end
